...
class API_BaseController {
	...
	private $apinamespaces;

	public $apinamespace = 'api/v1';

	function __construct()
	{
		$this->apinamespaces = 'api/v1';
	}
	...
	public function custom_validate_token( $server ){

		$headerkey = apply_filters( 'jwt_auth_authorization_header', 'HTTP_AUTHORIZATION' );
		$auth      = isset( $server[ $headerkey ] ) ? $server[ $headerkey ] : false;
	    if ( ! $auth ) {
	    	return -1;
	    }
		list($token) = sscanf( $auth, 'Bearer %s' );
		...
		if( isset($user_data->data->user->id)){
			$user_info = get_userdata( $user_data->data->user->id );
			$user_roles = $user_info->roles;
			if ( in_array( 'app_manager', $user_roles ) ) {
				$login_access = get_user_meta( $user_data->data->user->id , 'user_login_access', true );
				$user_created_by = get_user_meta( $user_data->data->user->id , 'user_created_by', true );
				if( ( '1' == $login_access) && ($user_created_by > 0) ){
			...
			return $user_data->data->user->id;
		}
		return -1;
	}
	public function check_user_permission(  $server, $permission_type ){
		...
		if( isset($user_data->data->user->id)){
			...
			if ( in_array( 'app_manager', $user_roles ) ) {
				...
				if( ( '1' == $login_access) && ($user_created_by > 0) ){
				 	...
				 	return ('view' == $permission_type ) ? true : false;  
				 	 
				}
			}else if ( in_array( 'app_admin', $user_roles ) ) {
				return true;
				 	 
			}else if ( in_array( 'administrator', $user_roles ) ) {
				return true;
				 	 
			}else if ( in_array( 'tenant', $user_roles ) ) {
				return true;
