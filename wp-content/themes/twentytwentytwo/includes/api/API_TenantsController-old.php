...
class API_TenantsControler extends API_BaseController {
   ...
   //  	foreach( $search_by_fields as $search_by_field ){
	 ...
	 */
	public function cloudemail_send_create_passowrd_email( $tenant_id, $password ){
		 
		$my_app_url =  get_option( 'my_app_url' );
		$admin_email =  get_option( 'admin_email' );
		$from = "info@getsmiapp.com";
		
		$company_name = get_user_meta( $company_name, 'company_name', true );
				
		$to =   get_post_meta( $tenant_id, 'primary_email', true);
		$to = trim( $to );
	...
	public function validate_tenant_password( WP_REST_Request $request ){
	    ...
	    $get_pass = get_post_meta( $tenant_id, 'create_password', true );
